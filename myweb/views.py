...
from myweb.models import Types,Goods,Users,Orders
...
def shoplist(request):
    ...
    if request.GET.get('tid','') != '':
		...
		# 相当于模糊查询 select * from goods where typeid in(select id from type where path like '%,1,%')
    context['goodslist'] = list
    return render(request,"myweb/shoplist.html",context)

#商品详情页
def shop(request,gid=0):
    context = loadinfo()
